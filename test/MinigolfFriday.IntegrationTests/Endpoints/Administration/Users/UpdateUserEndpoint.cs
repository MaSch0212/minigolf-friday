        ...
        (await sut.AppClient.GetUserAsync(user.Id))
          ...
          .BeEquivalentTo(
            new User()
            {
                ...
                Alias = user.Alias,
                Roles = user.Roles,
                PlayerPreferences = new() { Avoid = [users[2].User.Id], Prefer = [users[1].User.Id] }
            }
          );
    }
        ...
        (await sut.AppClient.GetUserAsync(user.Id))
          ...
          .BeEquivalentTo(
            new User()
            {
                ...
                Alias = user.Alias,
                Roles = user.Roles,
                PlayerPreferences = new() { Avoid = [users[0].User.Id], Prefer = [users[2].User.Id] }
            }
          );
    }
        ...
        (await sut.AppClient.GetUserAsync(user.Id))
          ...
          .BeEquivalentTo(
            new User()
            {
                ...
                Alias = "Renamed User",
                Roles = [Role.Admin],
                PlayerPreferences = new() { Avoid = [users[2].User.Id], Prefer = [users[3].User.Id] }
            }
          );
    }
}
